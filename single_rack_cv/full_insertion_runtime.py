#!/usr/bin/env python3
"""Production runtime for the full guarded RJ45 insertion sequence."""

from __future__ import annotations

import math

# Disable only the temporary 2 mm terminal cap before the scale-aware mount
# imports the connector-mode constants. The mesh-derived TCP, probe lock, and
# every controller safety threshold remain unchanged.
import connector_tcp_usd as _connector_tcp_usd

_connector_tcp_usd.PRECONTACT_ALIGNMENT_ONLY = False

import scale_aware_cable_mount as _scale_aware_cable_mount
...
class AngledHandStereoHandoffRuntime(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if mount is not None and getattr(mount, "tcp_probe_only", False):
            # A rejected mesh derivation remains locked by main.py before
            # detector initialization or any visual/motion command.
            return
