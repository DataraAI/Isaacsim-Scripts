#!/usr/bin/env python3
"""Benchmark-only SGBM diagnostic that preserves strict final qualification.

The v5 estimator aborted before recording accuracy whenever the independently
refined left/right cavity-center rays disagreed by more than 0.5 mm or the
final refitted plane exceeded 0.5 mm maximum residual by a small amount. This
wrapper relaxes only those two internal aborts so the frozen benchmark can
record the complete estimator output. The original strict benchmark gates are
then reapplied, plus an explicit 0.5 mm p95 plane-residual gate.

No relaxed setting is used by production control.
"""
...
from front_rim_sgbm import (
    DEFAULT_SGBM_CONFIG,
    estimate_front_plane_sgbm as _estimate_front_plane_sgbm,
)


STRICT_PLANE_RESIDUAL_P95_MM = 0.50
DIAGNOSTIC_SGBM_CONFIG = replace(
    DEFAULT_SGBM_CONFIG,
    center_max_gap_m=0.0020,
    plane_max_residual_m=0.0010,
)


def _diagnostic_estimate(*args, **kwargs):
    kwargs["cfg"] = DIAGNOSTIC_SGBM_CONFIG
    return _estimate_front_plane_sgbm(*args, **kwargs)
...
def _rewrite_report(summary: dict[str, object]) -> None:
    ...
    lines = ["FRONT-RIM LOCAL SGBM DIAGNOSTIC SUMMARY"]
    lines.extend(f"{key}={summary.get(key)}" for key in keys)
    lines.extend(
        (
            "",
            "Diagnostic-only internal gates:",
            "  center_max_gap_mm=2.0",
            "  plane_max_residual_mm=1.0",
            "",
            "Strict qualification remains:",
            "  ray_gap_p95_mm<=0.5",
...
def main() -> int:
    base.DEFAULT_SGBM_CONFIG = DIAGNOSTIC_SGBM_CONFIG
    base.estimate_front_plane_sgbm = _diagnostic_estimate
    ...
    if not summary_path.is_file() or not details_path.is_file():
        ...
        raise RuntimeError(
            "SGBM diagnostic base benchmark did not write summary/details output."
        )
    ...
    summary.update(
        {
            "schema_version": 4,
            "mode": "local_sgbm_diagnostic_v6",
            "base_status_before_postprocess": base_status,
            "diagnostic_internal_gates": {
                "center_max_gap_m": DIAGNOSTIC_SGBM_CONFIG.center_max_gap_m,
                "plane_max_residual_m": (
                    DIAGNOSTIC_SGBM_CONFIG.plane_max_residual_m
                ),
            },
