...
class CameraConfig:
    ...
    left_local_position: tuple[float, float, float] = (0.04, -0.020, 0.025)
    right_local_position: tuple[float, float, float] = (0.04, +0.020, 0.025)
    virtual_local_position: tuple[float, float, float] = (0.04, 0.0, 0.025)
...
class YOLOEConfig:
    enabled: bool = True
    model_name: str = str(
        Path(__file__).resolve().parent / "assets" / "models" / "yoloe-26l-seg.pt"
    )
    ...
    refine_percentiles: tuple[float, ...] = (15.0, 20.0, 25.0, 30.0)
...
class CableMountConfig:
    """Permanent pregrasped deformable-cable mount settings."""
...
class IKConfig:
    ...
    tool_center_local_position_m: tuple[float, float, float] = (0.0, 0.0, 0.1034)
    tool_center_local_orientation_wxyz: tuple[float, float, float, float] = (
        1.0,
        0.0,
        0.0,
        0.0,
    ...
    initial_position: tuple[float, float, float] = (0.9000, -0.1375, 1.3000)
...
class DebugConfig:
    ...
    estimated_port_marker_color: tuple[float, float, float] = (0.0, 1.0, 0.0)
...
class Config:
    ...
    front_plane: FrontPlaneRuntimeConfig = field(default_factory=FrontPlaneRuntimeConfig)
    cable_mount: CableMountConfig = field(default_factory=CableMountConfig)
    ik: IKConfig = field(default_factory=IKConfig)
    drive_tuning: DriveTuningConfig = field(default_factory=DriveTuningConfig)
