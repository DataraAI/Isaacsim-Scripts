...
DOWN_ORI = np.array([0.0, 1.0, 0.0, 0.0], dtype=np.float64)
...
def queue_pickup():
    ...
    controller.add_cartesian_waypoint(above, DOWN_ORI, max_frames=30, pos_tolerance=0.001, label="above")
    controller.add_cartesian_waypoint(at_connector, DOWN_ORI, max_frames=30, pos_tolerance=0.001, label="at_connector")
    controller.add_gripper_command(action="close", wait_frames=CLOSE_WAIT_FRAMES)
    controller.add_cartesian_waypoint(above, DOWN_ORI, max_frames=30, pos_tolerance=0.001, hold_gripper=True, label="lift")
