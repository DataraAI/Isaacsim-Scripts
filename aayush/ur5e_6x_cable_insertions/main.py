"""Six UR5e behaviour-tree demo: each arm grasps a cable and inserts into an RJ45.
...
from ur5e_6x_cable_insertions.scene import apply_ur5e_home_pose, build_scene
...
def _make_registry():
    return {
        ...
        "maneuver_to_port_offset": controller_primitive(
            queue_port_offset,
        ...
        "return_home": controller_primitive(queue_return_home, validate=check_at_home),
...
def main() -> int:
    ...
    for warm in range(240):
        ...
        if ready:
            for station in bundle.stations:
                apply_ur5e_home_pose(station.robot, apply_live=True)
            physics_ready = True
            print(f"[BT CABLE UR5E 6X] Physics view ready after {warm + 1} warmup step(s)")
            break
    if not physics_ready:
        print("[BT CABLE UR5E 6X FAIL] Physics Simulation View never became ready for joint reads.")
        _hold_gui(10.0)
        simulation_app.close()
        return 3
        ...
        for tree in trees:
            ...
            if tree.services.get("abort_simulation"):
                ...
                if result is Status.SUCCESS:
                    print(f"[BT CABLE UR5E 6X PASS {sid}] {tree.step_index} generated steps")
                else:
    ...
    if failures or running:
        ...
        if running:
            print(f"[BT CABLE UR5E 6X FAIL] Timed out after {frame} frames")
    else:
        print(f"[BT CABLE UR5E 6X PASS] All {len(successes)} station(s) completed in {frame} frames")
        exit_code = 0
