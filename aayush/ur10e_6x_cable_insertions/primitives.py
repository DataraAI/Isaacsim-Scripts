...
def meters_per_unit(stage) -> float:
    """USD stage metersPerUnit (DataHall_6r is authored in centimeters)."""
...
def prim_bbox(stage, prim_path: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """World AABB in **meters**, regardless of stage unit."""
...
def _station_tag(context) -> str:
    sid = context.services.get("station_id")
...
def detect_grasp_part(context) -> bool:
    ...
    if center[2] < block_top - 0.25 or center[2] > block_top + 0.35:
        print(
            f"[BT PERCEPTION{_station_tag(context)}] REJECT bad detection "
    ...
    print(f"[BT PERCEPTION{_station_tag(context)}] E_part006_44 @ {np.round(center, 4)}")
    return True
...
def check_physical_grasp(context) -> bool:
    ...
    try:
        ...
        print(f"[BT GRASP{_station_tag(context)}] validate bbox failed: {exc}")
        return False
...
def inspect_workspace(context) -> bool:
    ...
    ok = jp is not None
    print(f"[BT INSPECT{_station_tag(context)}] robot_state_valid={ok}")
    return bool(ok)
...
def _port_contacts_path(context) -> str:
    path = context.services.get("port_contacts_path")
    if not path:
        raise RuntimeError("services['port_contacts_path'] is required")
    return str(path)
...
def compute_port_targets(
    ...
    if not contacts:
        raise RuntimeError("Port contacts path was not provided")
...
def queue_port_approach(context) -> None:
    ...
    insert, approach, path_a, path_b = compute_port_targets(stage, _port_contacts_path(context))
    ...
    print(
        f"[BT PORT{_station_tag(context)}] staged transit {path_a} / {path_b}\n"
...
def check_at_port_approach(context) -> bool:
    ...
    if approach is None:
        try:
            _insert, approach, _a, _b = compute_port_targets(stage, _port_contacts_path(context))
            context.services["port_approach_point"] = approach.copy()
        except Exception as exc:
            print(f"[BT PORT{_station_tag(context)}] approach validate: cannot resolve port: {exc}")
            return False
    ...
    try:
        ...
        print(f"[BT PORT{_station_tag(context)}] approach validate: tip bbox failed: {exc}")
        return False
    ...
    print(
        f"[BT PORT{_station_tag(context)}] approach validate tip={np.round(tip, 4)} "
...
def check_at_port_insert(context) -> bool:
    ...
    if insert is None or approach is None:
        try:
            insert, approach, _a, _b = compute_port_targets(stage, _port_contacts_path(context))
            context.services["port_insert_point"] = insert.copy()
            context.services["port_approach_point"] = approach.copy()
        except Exception as exc:
            print(f"[BT PORT{_station_tag(context)}] insert validate: cannot resolve port: {exc}")
            return False
    ...
    try:
        ...
        print(f"[BT PORT{_station_tag(context)}] insert validate: tip bbox failed: {exc}")
        return False
    ...
    print(
        f"[BT PORT{_station_tag(context)}] insert validate tip={np.round(tip, 4)} "
...
def _log_ik_progress(context) -> None:
    ...
    if prev is None:
        ...
        if nxt is not None:
            print(
                f"[BT IK{_station_tag(context)}] NEXT [{nxt.get('label', '')}] "
    ...
    for i in range(prev, min(idx, len(queue))):
        ...
        if cmd.get("type") != "cartesian":
            continue
        print(
            f"[BT IK{_station_tag(context)}] REACHED [{cmd.get('label', '')}] "
        ...
        if context.services.get("monitor_cable_hold") and cmd.get("hold_gripper"):
            ...
            print(f"[BT GRIP{_station_tag(context)}] after REACHED [{cmd.get('label', '')}]: {_format_cable_status(info)}")
            if not held:
                _abort_cable_lost(
    ...
    if controller.is_done():
        print(f"[BT IK{_station_tag(context)}] NEXT (none — queue end)")
    else:
        ...
        if nxt is not None:
            ...
            print(f"[BT IK{_station_tag(context)}] NEXT (non-cartesian: {kind})")
    context.services["_ik_cmd_idx"] = idx
...
def _abort_cable_lost(context, info: dict, *, where: str) -> None:
    msg = (
        ...
        f"{_format_cable_status(info)}. Station aborted (other arms continue)."
    )
    print(msg)
