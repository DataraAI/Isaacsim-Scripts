...
def create_target_cube(stage: Usd.Stage) -> None:
    """Create /World/Cube xform and red cube mesh. World pose is set via SingleXFormPrim.set_world_pose (not USD translate)."""
    print(f"Creating target cube assets for port reference: {PORT_PRIM_PATH}")
    ...
    print(
        f"Target xform {TARGET_PRIM_PATH}: cube size 2, material red (invisible). "
        f"World pose is applied with set_target_world_pose after SingleXFormPrim.initialize()."
    )


def set_target_world_pose(
    target: SingleXFormPrim, world_position: typing.Union[np.ndarray, typing.Sequence[float]]
) -> None:
    """
    Set goal xform in world space. Using xformOp:translate with world coordinates is wrong when the
    parent (e.g. /World) has scale/rotation — that stores local translation, not world position.
    This matches robot.get_world_pose() and what RmpFlow reads via update_world().
    """
    pos = np.asarray(world_position, dtype=np.float64).reshape(3)
    _, orient = target.get_world_pose()
    orient = np.asarray(orient, dtype=np.float64).reshape(4)
    target.set_world_pose(position=pos, orientation=orient)
...
def step_follow_target(world, robot, ik_solver, lula_solver, target: SingleXFormPrim):
    ...
    target_pos = np.asarray(target.get_world_pose()[0], dtype=np.float64).reshape(3)
...
def update(
    ...
    if playing and n_ports >= 1:
        ...
        if n_ports == 1:
            ...
            set_target_world_pose(target, cube_pos)
        else:
    ...
    # fighting a mismatched default orientation. Read world position from the same SingleXFormPrim we
    # set with set_target_world_pose (matches robot.get_world_pose / RmpFlow, not raw local translate).
    cube_position = np.asarray(target.get_world_pose()[0], dtype=np.float64).reshape(3)
    base_pos, base_quat = robot.get_world_pose()
    lula_solver.set_robot_base_pose(base_pos, base_quat)
    ee_pos, _ = ik_solver.compute_end_effector_pose()

    motion_state["ee_dist_log_step"] = int(motion_state.get("ee_dist_log_step", 0)) + 1
    if motion_state["ee_dist_log_step"] % 60 == 0:
        ee_xyz = np.asarray(ee_pos, dtype=np.float64).reshape(-1)[:3]
        dist_m = float(np.linalg.norm(ee_xyz - cube_position))
        print(f"End effector distance to target (Lula EE vs target world pose): {dist_m:.4f}")
...
def move_target_and_follow(world, robot, ik_solver, lula_solver, target: SingleXFormPrim, start_pos, end_pos, steps):
    success_count = 0
    for i in range(steps):
        alpha = float(i + 1) / float(steps)
        target_pos = (1.0 - alpha) * start_pos + alpha * end_pos
        set_target_world_pose(target, target_pos)
        if step_follow_target(world, robot, ik_solver, lula_solver, target):
            success_count += 1
...
def main():
    ...
    lula_solver = LulaKinematicsSolver(
        **interface_config_loader.load_supported_lula_kinematics_solver_config("Franka")
    )
    ...
    target.initialize()
    set_target_world_pose(target, goal_position_for_port_index(stage, 0))
    ...
    #     step_follow_target(world, franka, ik_solver, lula_solver, target)
    #     world.step(render=True)
